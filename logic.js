...
/*This variable will be used to monitor if the user already won */
/*If one of these variables value become true, it means the player already won*/
...
/*Functios are callable programmed tasks*/
...
function setGame(){
	...
	for(let r=0; r < rows; r++){
		for(let c=0; c < columns; c++){
			...
			/*Use the number to repeat the tile's appearance*/
			updateTile(tile,num);
	...
	for(let i = 0; i < row.length - 1; i++){
        ...
        */
        ...
        if(row[i] == row[i+1]){
            ...
            row[i+1] = 0;
            score += row[i]; /*To change our score  */     
        } // [2, 2, 2] -> [4, 0, 2]
...
function slideLeft(){
	...
	for(let r=0; r < rows; r++){
		...
		/*All tiles value per row are saved in a container*/
		let row = board[r];
...
// input targets any elements that includes the word tile in their class 
